...
// --- PHẦN 2: MENU ITEMS (MÓN ĂN) ---

// Lấy danh sách món ăn (Có lọc theo Category)
exports.getMenuItems = async (req, res) => {
  const { category_id } = req.query; // Lấy từ URL ?category_id=1
  
  try {
    let query = 'SELECT * FROM menu_items WHERE status != $1 ORDER BY id DESC';
    let params = ['deleted']; // Không lấy món đã xóa

    if (category_id) {
      query = 'SELECT * FROM menu_items WHERE category_id = $1 AND status != $2 ORDER BY id DESC';
      params = [category_id, 'deleted'];
    }
...
exports.createMenuItem = async (req, res) => {
  ...
  // Lấy link ảnh từ Cloudinary (do middleware upload xử lý trước đó)
  const imageUrl = req.file ? req.file.path : null;

  try {
    // 1. Thêm vào bảng menu_items
    const result = await db.query(
    ...
    // 2. (Optional) Thêm vào bảng menu_item_photos nếu muốn lưu lịch sử ảnh
    if (imageUrl) {
        await db.query(
            'INSERT INTO menu_item_photos (menu_item_id, url, is_primary) VALUES ($1, $2, $3)',
            [newItem.id, imageUrl, true]
        );
    }
...
// Xóa món ăn (Soft Delete - chỉ chuyển trạng thái ẩn đi)
exports.deleteMenuItem = async (req, res) => {
    const { id } = req.params;
    try {
        await db.query("UPDATE menu_items SET status = 'deleted' WHERE id = $1", [id]);
